...
int xubuntu_install(progress_cb_t cb, void *userdata) {
    dl_ctx_t ctx = { cb, userdata };
    char tmp_archive[512];
    snprintf(tmp_archive, sizeof(tmp_archive), "%s%s", INSTALL_PATH_TMP, XUBUNTU_ROOTFS_FILE);

    fs_mkdir_p(INSTALL_PATH_XUBUNTU);
    fs_mkdir_p(INSTALL_PATH_TMP);

    if (cb) cb(0, 1000000, userdata);
    if (download_file(XUBUNTU_ROOTFS_URL, tmp_archive, dl_progress, &ctx) != 0) {
        cleanup_failed_install(INSTALL_PATH_XUBUNTU);
        return -1;
    }

    if (cb) cb(620000, 1000000, userdata);
    if (sha256_verify(tmp_archive, XUBUNTU_ROOTFS_SHA256) == 0) {
        fs_remove(tmp_archive);
        cleanup_failed_install(INSTALL_PATH_XUBUNTU);
        return -2;
    }

    if (fs_extract_tgz(tmp_archive, INSTALL_PATH_XUBUNTU, extract_progress, &ctx) != 0) {
        fs_remove(tmp_archive);
        cleanup_failed_install(INSTALL_PATH_XUBUNTU);
        return -3;
    }
    ...
    snprintf(kernel_src, sizeof(kernel_src), "%s%s", ASSET_PATH_KERNELS, XUBUNTU_KERNEL_FILE);
    snprintf(kernel_dst, sizeof(kernel_dst), "%s%s", INSTALL_PATH_KERNELS, XUBUNTU_KERNEL_FILE);
    fs_mkdir_p(INSTALL_PATH_KERNELS);
    if (fs_copy(kernel_src, kernel_dst) != 0) return -4;

    char cfg_path[512];
    snprintf(cfg_path, sizeof(cfg_path), "%sboot.cfg", INSTALL_PATH_XUBUNTU);
    FILE *cfg = fopen(cfg_path, "w");
    if (cfg) {
        fprintf(cfg, "distro=xubuntu\n");
        fprintf(cfg, "kernel=%s%s\n", INSTALL_PATH_KERNELS, XUBUNTU_KERNEL_FILE);
        fprintf(cfg, "rootfs=%s\n", INSTALL_PATH_XUBUNTU);
        fclose(cfg);
    }
